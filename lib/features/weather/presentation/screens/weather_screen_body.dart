...
import 'get_prediction_bloc_listener.dart';
...
class _WeatherScreenBodyState extends State<WeatherScreenBody> {
  ...
  void _updateDay(DateTime newDay) {
    ...
    weatherCubit.fetchWeather(city, selectedDay).then((_) {
      if (weatherCubit.weatherList.isNotEmpty) {
        final weatherData = weatherCubit.weatherList[weatherCubit.selectedIndex];
        weatherCubit.getTennisPredictionData();
      }
    });
  }
    ...
    return AnimatedContainer(
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(width: 5.h),
          Text(
           " ${AppStrings.Checkingweatherin} $cityName",
